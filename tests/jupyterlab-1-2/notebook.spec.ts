  ...
  for (const [file, sample] of parameters) {
    test(`measure ${file} - ${sample + 1}`, async ({
      ...
      const openTime = await perf.measure(async () => {
        ...
        await Promise.all([
          page.waitForSelector(".p-DockPanel >> .jp-SpinnerContent"),
          page.dblclick(`#filebrowser >> text=${filename}.ipynb`),
        ]);

        // Wait for spinner to be hidden
        await page.waitForSelector(".p-DockPanel >> .jp-SpinnerContent", {
          state: "hidden",
        });
      });

      // Check the notebook is correctly opened
      let panel = await page.$(".p-DockPanel >> .jp-NotebookPanel >> nth=0");
      await generators[file].waitFor({
        widgetID: await panel.getAttribute("id"),
        page,
      });
      ...
      await page.waitForSelector(
        `div.p-DockPanel >> .p-DockPanel-tabBar >> text=${path.basename(
          textFile
        )}`
      );
      ...
      await page.click(
        `div.p-DockPanel >> .p-DockPanel-tabBar >> text=${filename}.ipynb`
      );
      ...
      for (let switchIdx = 0; switchIdx < SWITCHES; switchIdx++) {
        if (STEPS.includes("switch-with-copy")) {
          ...
          const fromTimeCopy = await perf.measure(async () => {
            await page.click(
              `div.p-DockPanel >> .p-DockPanel-tabBar >> text=${filename}_copy.ipynb`
            );
          });

          // Check the notebook is correctly opened
          panel = await page.$(
            '.p-DockPanel >> .jp-NotebookPanel >> nth=1'
          );
          await generators[file].waitFor({
            widgetID: await panel.getAttribute("id"),
            page,
          ...
          // Check the notebook is correctly opened
          panel = await page.$(".p-DockPanel >> .jp-NotebookPanel >> nth=0");
          await generators[file].waitFor({
            widgetID: await panel.getAttribute("id"),
            page,
          });
        ...
        if (STEPS.includes("switch-with-txt")) {
          ...
          const editorPanel = page.locator(
            'div.jp-FileEditor:has-text("Lorem ipsum dolor sit amet, consectetur adipiscing elit. Proin mole")'
          );
      ...
      if (searchWord && STEPS.includes("search")) {
        await page.click('li.p-MenuBar-item:has-text("Edit")');
        await page.click('ul.p-Menu-content >> text=Find…');

        // Force searching in cell outputs
        await page.click('button[title="Show Search Filters"]');
        if (
          !(await page
            .locator('text=Search Cell Outputs >> input[type="checkbox"]')
            .isChecked())
        ) {
      ...
      await page.click('li.p-MenuBar-item:has-text("Kernel")');
      await page.click('ul.p-Menu-content >> text=Shut Down All Kernels…');
      await page.click('.jp-Dialog-footer >> button:has-text("Shut Down All")');

      if (STEPS.includes("close")) {
        // Close notebook
        await page.click('li.p-MenuBar-item:has-text("File")');
        const closeTime = await perf.measure(async () => {
          await page.click('ul.p-Menu-content >> text=Close Tab');
          // Revert changes so we don't measure saving
          const dimissButton = page.locator('button:has-text("OK")');
